...
export function productsRoutes() {
    ...
    productsRouter.get('/products', permit('user', 'manager', 'admin'), async (req, res) => {
        try {
            const { pageSize = 15, pageNumber = 1, sort, search, onlyHidden, onlyOutOfStock, onlyOpenedPackages, onlyOnSale, category, page, season } = req.query;

            let sortParam = sort;

            if (sort) {
                if (sort.startsWith('-')) {
                    sortParam = { [sort.substring(1)]: -1 };
                } else {
                    sortParam = { [sort]: 1 };
                }
            }

            const { count, products, pageCount, status, message } = await ProductController.get({ page, pageSize, pageNumber, search, sort: sortParam, onlyHidden, onlyOpenedPackages, onlyOnSale, onlyOutOfStock, category, season });
            if (status !== 200)
                return res.status(status).send(message);
    ...
    productsRouter.post('/products', permit('user', 'manager', 'admin'), imageUploader.fields([{ name: 'image', maxCount: 1 }, { name: 'additionalImages', maxCount: 10 }]), async (req, res) => {
        try {
            ...
            if (product.hidden === false)// if product should be hidden from website
            ...
            if (data.printLabel && io.sockets.adapter.rooms.get("printer") !== undefined)
                io.in('printer').emit('print', { name: product.name, code: product.code, barcode: product.barcode, retailPrice: product.retailPrice, wholesalePrice: product.wholesalePrice, sizes: product.sizes, multiplier: product.multiplier }, product.quantity);
    ...
    productsRouter.post('/products/restock', permit('user', 'manager', 'admin'), async (req, res) => {
        try {
            ...
            if (printLabelCheck && io.sockets.adapter.rooms.get("printer") !== undefined) {
                const productsToPrint = await Product.find({ _id: { $in: doneProducts.map(p => p._id) } }).select('name code barcode sizes wholesalePrice retailPrice multiplier');
    ...
    productsRouter.post('/products/revision', permit('admin'), async (req, res) => {
        try {
            ...
            const { doneProducts, status, message } = await ProductController.revision(products);
            if (status !== 200)
                return res.status(status).send(message);
