...
export const ProductController = {
    ...
    get: async ({ pageNumber, pageSize, page, sort, search, onlyHidden, season, onlyOutOfStock, onlyOpenedPackages, onlyOnSale, category }) => {
        ...
        if (page && page === 'orders') {
            ...
            let products = await Product.find({ inRevision: true, deleted: false, outOfStock: false }).sort({ category: 1, updatedAt: -1, }).select('name code barcode quantity sizes.size sizes.quantity deliveryPrice wholesalePrice upsaleAmount multiplier');
            return { products, status: 200 };
        }
    ...
    delete: async (id) => {
        ...
        const wooData = product.woocommerce;
