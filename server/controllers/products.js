...
export const ProductController = {
    ...
    get: async ({ pageNumber, pageSize, page, search, onlyHidden, season, onlyOutOfStock, onlyOpenedPackages, onlyOnSale, category }) => {
        ...
        if (page && page === 'orders') {
            ...
            let products = await Product.find({ inRevision: true, deleted: false, outOfStock: false }).sort({ category: 1, updatedAt: -1, }).select('name code barcode quantity sizes.size sizes.quantity deliveryPrice wholesalePrice upsaleAmount');
            return { products, status: 200 };
        }
